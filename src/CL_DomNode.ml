...
module Exceptionless = struct
  ...
  let get_first_child node =
    try Some (get_first_child node)
    with Not_found -> None

  let get_last_child node =
    try Some (get_last_child node)
    with Not_found -> None

  let get_next_sibling node =
    try Some (get_next_sibling node)
    with Not_found -> None

  let get_previous_sibling node =
    try Some (get_previous_sibling node)
    with Not_found -> None

  let get_parent_node node =
    try Some (get_parent_node node)
    with Not_found -> None
end
