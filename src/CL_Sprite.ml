...
let init_with ~gc ~src =
  match src with
  | `Fullname fullname -> init_with_gc_and_fullname ~gc ~fullname
  | `File_dir_src { filename; dir } ->
      init_with_gc_filename_vdir ~gc ~filename ~dir
  | `IO { image_in = file; image_type } ->
      init_with_gc_and_io ~gc ~file ~image_type
